...
const SingleFeaturedProduct = ({ product, refetch, showTrendingBadge }) => {
  ...
  return (
    <li className="rounded-xl dark:bg-base-200 z-10 p-4 shadow-sm hover:shadow-md  transition flex flex-col gap-3 h-full">
      ...
      {/* Bottom actions - Outside of Link */}
      <div className="flex items-center justify-between mt-auto pt-2 text-sm z-20 text-gray-500">
        <button
          disabled={user?.email === creator_email}
          onClick={handleUpvote}
          className={`flex btn items-center gap-2 ${
            data?.hasUpvoted ? 'bg-blue-400 text-white' : ''
          }`}
        >
